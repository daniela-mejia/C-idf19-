...
int main()
{
  ...
  // TODO: Create a char buffer named *buffer using malloc ensure it is large enough to hold string[] + a nul-terminator
  char *buffer;
  // need to give some memory to the buffer 
  buffer = malloc(sizeof (char) * 256);

  // TODO: Ensure buffer was created
  while ( buffer == NULL)
  {
    printf(" Error no hay out stack \n");
    return 0;
  }
...
int haystack_needle (char *haystack, char *needle, char *buffer)
{
    ...
    char *mark_position = strst (haystack, needle)      //Returns pointer to first occurrence of str2 (needle) in str1 (haystack)

    // TODO: Using strcpy(), store the needle into the buffer... \
    you are not authorized to use the variable needle.
    
    strcpy ( buffer, mark_position);    //above needle str2 is in haystack str1
    ...
    memmove((void*) mark_position, (void*)(mark_position + needleLength), needleLength);   //memmove() is used to copy a block of memory from a location to another.
    return 0;
}