...
*/
#include <stdio.h>
#include <string.h>/* Error code default value */
...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)     //Null terminated , Null but inside sentence_ptr, Store your error code at this memory address.
{
	if(sentence_ptr == NULL)                                //sentence_ptr is a char pointer to a null-terminated string to search
    {
        *errorCode_ptr = ERROR_NULL_SENTENCE_POINTER;       //define previously ((int)-1)
        return NULL;
    }
    else if(searchWord_ptr == NULL)                         // char pointer to the first occurrence of the searchWord_ptr string found in the sentence_ptr string
    {
        *errorCode_ptr = ERROR_NULL_SEARCH_POINTER;       	// define previously ((int)-2)
        return NULL;
    }
    else if(errorCode_ptr == NULL)                          //If null errorCode is passed,ERROR and return NULL
    {
        return NULL;  										//the searchWord_ptr string is not found in the sentence_ptr string
    }
    else
    {
        return 0;
    }
  //The char pointer should be a memory address in the
 //*sentence_ptr string and this memory address should represent the first 'index'
 //*of the searchWord_ptr string.
	for(int i = 0; i < strlen(sentence_ptr); i++)             //i less than the total lenght of the sentence_ptr and increase i every time it loops
    {	
        ...
        if((strlen((sentence_ptr + i)) - strlen(searchWord_ptr)) >= 0)    //If our search word fits then search for it
        {
            for(int j = 0; j < strlen(searchWord_ptr); j++)         //For every letter in search word
            {
                ...
                if (*(sentence_ptr + i + j) != *(searchWord_ptr + j))        //If the letters aren't the same then break *value
				{
                    return 0;
                }
                else if(j == strlen(searchWord_ptr) - 1)    //if the value es less than de address then ERROR
                {
                    *errorCode_ptr = ERROR_CODE_SUCCESS;
                    return &sentence_ptr[i];
                }
            }
        }
    }
