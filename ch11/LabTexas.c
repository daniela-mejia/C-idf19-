...
//pass by value will only send the first value in an array, pass by reference sends the whole array. Pointers pass by reference.
int main(void)
{
    int arrayOfInts [] = {1,2,3,4,5,6,7}; // a pointer to an array of integers
	int numOfElements = 7;  //the dimension of the integer array found at the int pointer arrayOfInts
    int (* compare_function[4])  (int, int) = {low_to_high_compare, high_to_low_compare, absolute_low_to_high_compare, absolute_high_to_low_compare}; 
    
    //Initializes a pointer that points to an array of functions. 
    //Functions need a pointer to pass a function bc if you 
    //pass by value
    for (long int j = 0; j < 4 ; j++) //max comparition Array index is 4 has to be less than the index to iterate 
    {
        bubble_sort( arrayOfInts, numOfElements, (*compare_function[j]));      //Bubble Sorts works by repeatedly swapping the elements.
        for(int i = 0; i < numOfElements; i++)  //iterate less than than numOfElements so the array can be print in the desire orders
        {
	...
	return 0; /// return to the top after operated 
}
 ...
 *                 compare_function will be considered an error value (see: RETURNS).
 */
int bubble_sort(int * arrayOfInts, int numOfElements, int(*compare_function)(int, int)) //functions need a pointer to pass a function bc if you 
//pass by value, it'll pass the return value, not the function itself
{
