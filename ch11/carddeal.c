...
int main()
{
   ...
   unsigned int hand[ 5 ][ 2 ];      // represents the hand
   size_t row, column;               // loop counters
   
   // loop through rows of deck
   for ( row = 0; row < SUITS; ++row )   // increment row
   {
      // loop through columns of deck for current row (this loop is in row!!!)
   ...
   srand( time( NULL ) ); // seed random number generator  {The srand() function sets the starting point for producing a series of pseudo-random integers}
