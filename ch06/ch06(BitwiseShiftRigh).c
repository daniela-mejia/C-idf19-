...
int main(void)
{
	uint32_t inputNumber = 0;           // Input a uint32_t
...
#ifdef solution1
	{
		...
		inputNumber = inputNumber >> 4;                     //Move the original by 4 positions

		printf("\tC. Bit shift the copy left by 32 - 4 = 28. \n"); //Drop the four right-most bits off the uint32_t using a Bitwise Shift Right >> 32-4=28
		transitionNumber = transitionNumber << ((sizeof(transitionNumber) * 8) - 4); //bits are 32
